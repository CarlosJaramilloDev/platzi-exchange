    ...
    <template v-if="asset.id && !isLoading">
      <div class="flex flex-col sm:flex-row justify-around items-center">
        <div class="flex flex-col items-center">
          <img v-bind:src="`https://static.coincap.io/assets/icons/${asset.symbol.toLowerCase()}@2x.png`" v-bind:alt="asset.name" class="w-20 h-20 mr-5" />
          <h1 class="text-5xl">
              {{ asset.name}}
      ...
      <line-chart class="my-10"
        ...
        v-bind:min="min"
        v-bind:max="max"
        v-bind:data="history.map(h => [h.date, parseFloat(h.priceUsd).toFixed(2)])"
      ...
      <table>
        ...
        > 
          ...
          <td>
            <px-button-programatic
            :is-loading="m.isLoading || false"
            v-if="!m.url"
            @click="getWebsite(m)"
            >Obtener link</px-button-programatic>
...
<script>
import PxButtonProgramatic from '@/components/PxButtonProgramatic'
import api from '@/api'
